...
只输出字段名，不要其他内容。"""
        ...
        try:
            result = await multi_llm.agenerate([{"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{image_data}"}}, {"type": "text", "text": prompt}])
            result_text = result.content if hasattr(result, 'content') else str(result)
            field_names = [line.strip() for line in result_text.strip().split('\n') if line.strip() and len(line.strip()) > 1]
    ...
    async def _extract_values(
        ...
        for i, fname in enumerate(field_names):
            ...
            cropped_img = cropped_img.resize(
                (cropped_img.width * 5, cropped_img.height * 5), Image.LANCZOS
            )
