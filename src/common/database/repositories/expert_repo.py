...
class XkflRepository:
    ...
    def list_all_zrjj(self) -> List[dict]:
        """查询所有自然科学奖学科分类 (来自 kjjhxm_wlps.sys_xkfl_zrjj)"""
        conn = get_project_connection()
        try:
            with conn.cursor() as cursor:
                cursor.execute("SELECT code, name FROM sys_xkfl_zrjj ORDER BY code")
                rows = cursor.fetchall()
                return [{"code": r[0], "name": r[1]} for r in rows]
        finally:
            conn.close()

    def get_by_code_zrjj(self, code: str) -> Optional[dict]:
        """根据代码查询自然科学奖学科分类"""
        conn = get_project_connection()
        try:
            with conn.cursor() as cursor:
                cursor.execute("SELECT code, name FROM sys_xkfl_zrjj WHERE code = ?", (code,))
                row = cursor.fetchone()
                if row:
