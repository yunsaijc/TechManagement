...
async def get_knowledge_graph(query: str = "", year: int | None = None, limit: int = 80, offset: int = 0, view: str = "all", start: str = "project") -> dict[str, object]:
    """读取 192.168.0.198 Neo4j 中的项目中心知识图谱子图。"""
    from src.services.sandbox.knowledge_graph import load_knowledge_graph

    try:
        return await run_in_threadpool(load_knowledge_graph, query=query, year=year, limit=limit, offset=offset, view=view, start=start)
    except RuntimeError as exc:
        raise HTTPException(status_code=500, detail=str(exc)) from exc
    except Exception as exc:
        raise HTTPException(status_code=502, detail=f"Neo4j 图谱读取失败：{exc}") from exc


@router.get("/knowledge-graph/wide")
async def get_knowledge_graph_wide(
    query: str = "",
    year: int | None = None,
    ...
    nodeLimit: int = 9000,
    edgeLimit: int = 14000,
    timeoutSeconds: float = 18.0,
    taskOffset: int = 0,
    taskLimit: int = 0,
) -> dict[str, object]:
    """后端聚合式读取 Neo4j 全库广域样本，供前端一次性展示更多节点和边。"""
    from src.services.sandbox.knowledge_graph import load_knowledge_graph_wide

    try:
        return await run_in_threadpool(
            load_knowledge_graph_wide,
            query=query,
            year=year,
            full_library=fullLibrary,
            batch_limit=batchLimit,
            pages_per_combo=pagesPerCombo,
        ...
        raise HTTPException(status_code=502, detail=f"Neo4j 广域图谱读取失败：{exc}") from exc


@router.get("/knowledge-graph/overview")
async def get_knowledge_graph_overview(samplePerLabel: int = 24, samplePerRelation: int = 12) -> dict[str, object]:
    """快速读取 Neo4j 全库概览：全局类型计数 + 少量代表样本。"""
    from src.services.sandbox.knowledge_graph import load_knowledge_graph_overview

    try:
        return await run_in_threadpool(
            load_knowledge_graph_overview,
            sample_per_label=samplePerLabel,
            sample_per_relation=samplePerRelation,
        )
    except RuntimeError as exc:
        raise HTTPException(status_code=500, detail=str(exc)) from exc
    except Exception as exc:
        raise HTTPException(status_code=502, detail=f"Neo4j 全库概览读取失败：{exc}") from exc


@router.get("/knowledge-graph/type")
async def get_knowledge_graph_type(
    typeLabel: str,
    limit: int = 80,
    offset: int = 0,
    query: str = "",
    ...
    view: str = "all",
    start: str = "project",
    timeoutSeconds: float = 28.0,
    contextElementIds: str = "",
) -> dict[str, object]:
    """展开某个聚合簇，读取该类型的一批真实节点。"""
    from src.services.sandbox.knowledge_graph import load_knowledge_graph_nodes_by_type

    context_ids = [item.strip() for item in contextElementIds.split(",") if item.strip()]
    try:
        return await run_in_threadpool(
            load_knowledge_graph_nodes_by_type,
            ...
            start=start,
            timeout_seconds=timeoutSeconds,
            context_element_ids=context_ids or None,
        )
        ...
        raise HTTPException(status_code=502, detail=f"Neo4j 类型节点读取失败：{exc}") from exc
...
async def get_knowledge_graph_neighbors(
    elementId: str,
    limit: int = 80,
    timeoutSeconds: float = 24.0,
) -> dict[str, object]:
    """展开一个真实节点的一跳邻居。"""
    from src.services.sandbox.knowledge_graph import load_knowledge_graph_node_neighbors

    try:
        return await run_in_threadpool(
            load_knowledge_graph_node_neighbors,
        ...
        raise HTTPException(status_code=502, detail=f"Neo4j 邻居节点读取失败：{exc}") from exc
