...
class SMBReviewFileReader:
    ...
    @staticmethod
    def _preferred_office_extension(file_path: str) -> str | None:
    ...
    @classmethod
    def _swap_office_extension(cls, file_path: str) -> str | None:
    ...
    def read_bytes(self, file_path: str) -> bytes:
        ...
        try:
            ...
            preferred = self._preferred_office_extension(file_path)
            swapped = self._swap_office_extension(file_path)
            should_try_swap = (
                swapped is not None
                and preferred is not None
                and str(file_path).lower().endswith(preferred) is False
            )

            if should_try_swap:
                alt_unc_path = self.build_unc_path(swapped)
                try:
                    with smbclient.open_file(alt_unc_path, mode="rb") as fp:
                        return fp.read()
                except Exception as alt_exc:
                    raise SMBFileReadError(
                        f"读取 SMB 文件失败: {unc_path}: {exc}; 同时尝试: {alt_unc_path}: {alt_exc}"
                    ) from alt_exc
