...
class RewardFieldResultAggregator(ResultAggregator):
    def __init__(self, section_extractor=None, template_filter=None):
        ...
        self._reward_doc_texts: Optional[Dict[str, str]] = None
    ...
    def aggregate(
        ...
        self._reward_doc_texts = doc_texts or {}
        try:
    ...
    def _source_excerpt_for_quality(self, match: Match) -> str:
        doc_texts = self._reward_doc_texts or {}
    ...
    def _format_segments(
        ...
        for seg in formatted:
            ...
            primary_start = int(seg.get("primary_start", 0) or 0)
            primary_end = int(seg.get("primary_end", 0) or 0)
            source_start = int(sources[0].get("start", 0) or 0)
            source_end = int(sources[0].get("end", 0) or 0)

            if primary_end <= primary_start or source_end <= source_start:
                split_segments.append(seg)
                continue

            if primary_end > len(primary_full) or source_end > len(source_full):
                split_segments.append(seg)
                continue

            primary_raw = primary_full[primary_start:primary_end]
            source_raw = source_full[source_start:source_end]
            if not primary_raw.strip() or not source_raw.strip():
                split_segments.append(seg)
                continue

            matcher = difflib.SequenceMatcher(None, primary_raw, source_raw)
            blocks = self._extract_line_safe_blocks(matcher, primary_raw, source_raw)

            if not blocks:
                split_segments.append(seg)
                continue

            primary_len = max(primary_end - primary_start, 1)
            base_ngram = int(seg.get("ngram_count", 0) or 0)
            for a_off, b_off, size in blocks:
                new_primary_start = primary_start + a_off
                new_primary_end = new_primary_start + size
                new_source_start = source_start + b_off
                new_source_end = new_source_start + size

                primary_line, primary_text = self._get_line_info(
                    doc_a, new_primary_start, new_primary_end, doc_texts
                )
                source_line = source_full[:new_source_start].count("\n") + 1 if new_source_start > 0 else 1
                source_text = source_full[new_source_start:new_source_end].replace("\n", " ").strip()
                ...
                if self._contains_substantial_english(primary_line_text) or self._contains_substantial_english(source_line_text):
                    if not self._english_lines_exact_match(primary_line_text, source_line_text):
                        continue
                    primary_line = primary_full[:primary_line_start].count("\n") + 1 if primary_line_start > 0 else 1
                    source_line = source_full[:source_line_start].count("\n") + 1 if source_line_start > 0 else 1
                    primary_text = primary_line_text
                    source_text = source_line_text
                    new_primary_start = primary_line_start
                    new_primary_end = primary_line_end
                    new_source_start = source_line_start
                    new_source_end = source_line_end
                ...
                cloned["primary_start"] = new_primary_start
                cloned["primary_end"] = new_primary_end
                cloned["char_count"] = max(0, len(primary_text))
                if base_ngram > 0:
                    cloned["ngram_count"] = max(1, int(round(base_ngram * (size / primary_len))))
                ...
                if not is_template:
                    primary_span_len = int(cloned.get("primary_end", 0) or 0) - int(cloned.get("primary_start", 0) or 0)
                    source_span_len = int(new_source_end) - int(new_source_start)
                ...
                cloned["is_template"] = is_template
                cloned["template_reason"] = template_reason
                split_segments.append(cloned)

        split_segments.sort(key=lambda item: (int(item.get("primary_start", 0) or 0), -int(item.get("char_count", 0) or 0)))
    ...
    @staticmethod
    def _meaningful_block_text(text: str) -> bool:
        ...
        return (len(cleaned) >= 12 and zh_count >= 8) or (alpha_count >= 24 and word_count >= 5)

    def _extract_line_safe_blocks(
        self,
        ...
        for block in matcher.get_matching_blocks():
            if block.size <= 0:
                continue
            matched_text = primary_raw[block.a : block.a + block.size]
            line_offset = 0
            for part in matched_text.split("\n"):
                part_len = len(part)
                if part_len > 0 and self._meaningful_block_text(part):
                    blocks.append((block.a + line_offset, block.b + line_offset, part_len))
                line_offset += part_len + 1
        return blocks
