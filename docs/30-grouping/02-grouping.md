...
3. 数量 < min_per_group(5) → 同学科小组合并
4. 阶段2：跨学科小项目归并（关键词预筛 + LLM 逐项目判断）
5. 输出最终分组结果
```
...
当某学科项目数少于 `min_per_group`（默认5）时，分两阶段处理：
...
对于阶段1处理后仍项目过少（< `min_per_group`）的组，将其中的跨学科项目逐个归并到主题相近的大组：

**Step 1 - 关键词预筛（快速过滤）**

```
小组成员: [项目A(关键词: AI, 机器学习), 项目B(关键词: 深度学习, NLP)]
大组候选: 大组1(关键词: 人工智能, 大数据), 大组2(关键词: 机械制造, 材料)

重叠计算:
- 大组1: 重叠词 ≥ 1 (AI/机器学习 vs 人工智能/大数据) → 候选
- 大组2: 重叠词 = 0 → 不考虑
```

- 提取每个小项目的关键词集合
- 与每个大组（count >= min_per_group）的关键词集合计算重叠词数
- 重叠词 >= 1 → 该大组进入候选列表

**Step 2 - LLM 逐项目判断归并（精细）**

对每个小项目，调用 LLM 判断其归属：

```
你是一个学科分类专家。判断以下项目更适合放入哪个评审组。
...
可选评审组：
1. [大组1名称] - 已有N项，关键词: xxx, xxx
2. [大组2名称] - 已有M项，关键词: xxx, xxx
...

请判断这个项目最适合放入哪个组（只能选一个），或者选择"不合并"（如果项目主题特殊，无法归入任何现有组）。
...
**处理规则**：
- LLM 判断"不合并" → 项目留在原小组（即使很小）
- LLM 判断某个大组 → 将项目移动到该大组
- 合并后记录日志，便于追溯
