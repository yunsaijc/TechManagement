...
- 保持现有聊天链路稳定，不为追求“最佳实践”重写已验证的低时延问答主路径
- 后续主改动集中在 `项目类型自适应 rubric`、`证据包构建`、`打分校准`、`结构化摘要`
- 所有增强能力统一建立在可跳转、可核验的正文证据之上
...
- 聊天能力当前以“体验稳定、引用可核验、首字延迟低”为第一目标，后续只做轻量增强，不重构主链路
- 结构化摘要、九维评分、总评结论需要继续向 `rubric-first + evidence-first` 收敛
...
- 画像不改变九维框架本身，但会影响 `rubric`、缺失项容忍规则、维度解释口径
- 当前优先解决“平台建设类/科普实施类项目不应因缺少独立技术路线而被机械低分”

### 5. 证据优先

- 摘要、评分、总评、问答统一复用同一套证据底座
- 每个维度先形成 `evidence pack`，再进入 LLM 判断
- 证据不足时允许保守评价或明确标注“材料未提供”，而不是机械给出确定性结论
...
- 正式 HTML 报告采用左右双栏：
  ...
  - 右侧：评审结果区
- 右侧聊天面板应提供过程进度、结构化回答卡和精简证据卡，避免调试式堆叠信息
