...
zxmc
  ...
  -> 生成项目审查 packet（原始材料按顺序合并）
```

## 报告查看原则

项目报告层不再让右栏在多个原始文件之间切换，而是为每个项目生成一个统一 `packet PDF`。

这个 `packet PDF` 的原则只有三条：

- 保留原始文件内容，不重排、不拆分业务结构
- 保持材料原始顺序：`申报书 -> 附件1 -> 附件2 -> ...`
- 规则点击只做“跳到 packet 的目标页”，不再切换不同文件查看器

说明：

- 若申报书同时存在 `pdf` 和 `docx`，`packet` 优先使用原始 `pdf`，以保证“原样查看”
- `docx` 仍用于事实提取和文本定位辅助，但不作为 packet 主视图的首选来源
- 若个别材料暂时无法合入 packet，报告层应明确显示降级说明，而不是静默跳到其他文件
