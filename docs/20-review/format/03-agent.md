...
ReviewAgent 是当前附件级形式审查的核心组件，协调规则引擎和多模态 LLM，实现单附件审查。

在推荐方案中，ReviewAgent 保持为附件级能力，不直接承担项目级材料齐套性和项目类型规则判断。
...
| 组件 | 职责 |
|------|------|
| 附件类型输入 | 接收调用方指定的附件类型；奖励平台路径接口使用 `doc_type` 字典值 |
| 内容提取器 | OCR 提取文字 + 结构化解析 |
| 规则执行器 | 加载并执行检查规则 |
| 结果聚合器 | 汇总单附件结果，生成总结和建议 |
...
- **项目级规则上移**：材料齐套性、项目类型差异、跨附件规则由上层项目级编排负责

## 当前实现约束

当前代码实现与早期设计稿有以下差异：

- `document_type` 在接口中为必填，由调用方指定
- 奖励平台 SMB 路径主线约定使用 `doc_type`，建议值为 `tjdwyj/gzdwyj/wcr/wjwcr/wcdw/hzdw`
- Agent 当前不承担完整项目级审查
- 规则体系当前以单附件检查为主
